...
int makingtheLinkedList(myInfoObject *myInfo)
{
	...
	{
		add_node_end(&listHead, environ[listSize], 0);
	}
...
char **gettingMyEnviron(myInfoObject *myInfo)
{
	if ((*myInfo).environment_changed == 1 || (*myInfo).myenviron == NULL)
	{
		(*myInfo).myenviron = convertingNodeStringstoArray((*myInfo).environment);
		(*myInfo).environment_changed = 0;
	}
	return ((*myInfo).myenviron);
}
...
void freeMyInfo(myInfoObject *myInfo, int allCondition)
{
	...
	if (allCondition != 0)
	{
		...
		if ((*myInfo).myenviron != NULL)
			freeAString((*myInfo).myenviron);
